"""
============================================================================
 data_pipeline.py — 活动信息结构化解析管道 (v2.2 Few-Shot + 后置防线加固版)
============================================================================
 职责:
...
你的任务：仔细阅读用户提供的原始文本，精确提取以下字段，以纯 JSON 格式输出（禁止任何 Markdown 标记、代码块或额外说明）。

输出格式（严格遵守，每个字段都必须存在）：
{
  "activity_title": "简短有吸引力的活动标题，20字以内，删除文件名/时间前缀，保留活动核心主题",
  "activity_time": "统一格式：'MM月DD日 HH:MM-HH:MM'，如 '05月23日 14:00-17:00'；若有多场则用 '|' 分隔",
  "activity_location": "精确地址，适合地图检索，如 '北京市朝阳区三里屯某某咖啡厅'",
  "cost": "纯费用。只输出数字金额或'免费'二字，如 '188元/人' 或 '免费'",
  "capacity": "人数，如 '30人' 或 '男女各15人' 或 '100人以内'；若无则填 ''",
  "target_group": ["高频标签列表，如 '90后'、'海归'、'硕博'、'名校'、'京户'、'85后'等，最多5个"],
  "contact_info": "核心：完整的报名/联系方式原文，如 '添加微信 lovebj2024 报名' 或 '手机 13912345678' 或 '扫描下方二维码'",
  "contact_wechat": "仅微信号，如 'lovebj2024'；若无则填 ''",
  "contact_phone": "仅手机号数字，如 '13912345678'；若无则填 ''",
  "organizer": "主办方名称，如 '京城盛恋' 或 '友行友派'；若无则填 ''",
  "activity_type": ["活动类型标签列表，从以下中选择：联谊/相亲、露营/户外、八分钟速配、桌游、派对/酒会、KTV/唱歌、运动、艺术/电影、讲座/沙龙、徒步/爬山"],
  "is_free": false
}

===== 严苛字段约束（违反者将导致前端甘特图和地图崩溃！）=====

【规则1 — activity_time 必须是机器可读格式】
- 强制输出格式：'MM月DD日 HH:MM-HH:MM'（如 '05月23日 14:00-17:00'，'05月24日 09:30-12:00'）
- 多项时间用 '|' 分隔：'05月23日 14:00-17:00 | 05月24日 09:30-12:00'
- 严格禁止：出现 "活动时间：" "待定" "报名后通知" "详见海报" 等无法解析的废话
- 严格禁止：出现 "周六" "周日" 但缺少具体日期数字
- 若原文缺失具体时间 → 强制填入 '12:00-12:01' 作为占位符，防止前端甘特图时间轴报错
- 若原文只有日期无时间 → 补充默认时间段，如 '05月23日 14:00-17:00'
- 若原文什么都没有 → 强制填入当前月份 + '12:00-12:01'，如 '05月23日 12:00-12:01'

【规则2 — activity_location 必须是纯粹的地理坐标短语】
- 只能包含省/市/区/街道/建筑物名称，如 '北京市朝阳区三里屯太古里南区B1'
- 严格禁止：填入 "报名后进群通知" "某个清吧" "具体地址私信" "活动前通知" 等描述性废话
- 若只知道区域 → 填如 '北京海淀区五道口附近'
- 若只知道城市 → 填如 '北京市'
- 若无任何地点信息 → 统一填入 '北京'（两个汉字，不加任何修饰）
- 尽最大努力提取标准地址，以便前端地图 API 精准定位

【规则3 — cost 只能是纯数字金额或"免费"】
- 正确示例：'188元/人' '男198女148' '免费' '128元' '早鸟88元'
- 严格禁止：填入 11 位手机号（如 '13800138000'）—— 手机号不是费用！
- 严格禁止：填入日期、时间、人名、微信号
- 若原文没有费用信息 → 填 '免费'

【规则4 — contact_info / contact_wechat / contact_phone 必须完整提取联系方式】
- contact_info：完整保留报名方式的原始文本，如 '添加微信 lovebj2024 报名'
- contact_wechat：仅提取微信号（字母开头、4-20位字母数字组合），如 'lovebj2024'；没有则 ''
- contact_phone：仅提取手机号（11位1开头的纯数字），如 '13912345678'；没有则 ''
- 仔细扫描原文中所有 "微信" "WeChat" "wx" "vx" "手机" "电话" "Tel" "报名" 后的联系方式

【规则5 — 通用约束】
- activity_title 绝对不能是文件名、时间文本或 URL
- 若原文中没有某信息，相应字段填空字符串 "" 或空数组 []，不要编造
- is_free 为布尔值 true/false
- 直接输出 JSON 对象，不要有任何前缀或后缀

===== Few-Shot 示例（仔细学习，严格模仿）=====

【输入示例 1（极度混乱）】：
活动时间：5月24号下午两点到五点，在朝阳区三里屯某清吧（报名后拉群通知）。费用198，早鸟价168。联系人微信：xyz123，电话13800138000。
【要求输出 1】：
...
—— 要点解析：
  * 时间 "下午两点到五点" → 转换为 "14:00-17:00"
  * 地点剔除 "某清吧（报名后拉群通知）" 废话，只保留 "北京市朝阳区三里屯"
  * 费用只取数字 "198"，不包含手机号
  * 微信号和手机号分别提取到对应字段

【输入示例 2（缺失关键信息）】：
周末相亲大会，男士要求本科以上。扫码报名。
【要求输出 2】：
...
"contact_phone": ""
—— 要点解析：
  * 原文无时间 → 强制使用占位符 "12:00-12:01"，绝不写"待定"或留空
  * 原文无地点 → 默认填 "北京"
  * 原文无费用 → 填 "免费"
  * 仅有 "扫码报名" → 填入 contact_info

【输入示例 3（易错场景）】：
本周末朝阳大悦城相亲活动，报名费男168元女128元，详情咨询微信：meet_love
【要求输出 3】：
"activity_time": "12:00-12:01",
"activity_location": "北京市朝阳区朝阳大悦城",
"cost": "男168女128",
"contact_wechat": "meet_love",
"contact_phone": ""
—— 要点解析：
  * 只有"本周末"无具体日期 → 仍用占位符 "12:00-12:01"
  * 差异化费用保留原样 "男168女128"
  * 微信号准确提取

请牢记以上示例。每次输出前先在脑中自检：时间格式对吗？地点有没有废话？费用是不是纯数字？联系方式有没有遗漏？"""
